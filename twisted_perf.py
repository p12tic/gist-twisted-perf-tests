...
def asyncToDeferred(f):
    @wraps(f)
    def wrapper(*args, **kwargs):
        return defer.ensureDeferred(f(*args, **kwargs))
    return wrapper

async def test_async_await_async(x):
    return await test_deferred(x)
...
def gen_test_yield_N_func_succeed_return(n):
    @defer.inlineCallbacks
    def test_yield_N_func_succeed_return(x):
        x = yield defer.succeed(x)
        for i in range(n):
            x = yield func(x)
        return x
    return test_yield_N_func_succeed_return

def gen_test_yield_N_func_return(n):
    @defer.inlineCallbacks
    def test_yield_N_func_return(x):
        for i in range(3):
            x = yield func(x)
        return x
    return test_yield_N_func_return
...
def gen_test_yield_N_succeed_deferred_return(n):
    @defer.inlineCallbacks
    def test_yield_N_succeed_deferred_return(x):
        x = yield defer.succeed(x)
        for i in range(n):
            x = yield test_deferred(x)
        return x
    return test_yield_N_succeed_deferred_return

def gen_test_yield_N_deferred_return(n):
    @defer.inlineCallbacks
    def test_yield_N_deferred_return(x):
        for i in range(n):
            x = yield test_deferred(x)
        return x
    return test_yield_N_deferred_return

def gen_test_yield_N_yield_M_func_return(n, m):
    ...
    @defer.inlineCallbacks
    def test_yield_N_yield_M_func_return(x):
        for i in range(n):
            x = yield impl2(x)
        return x
    return test_yield_N_yield_M_func_return

def gen_test_await_N_await_M_func_return(n, m):

    @asyncToDeferred
    async def impl1(x):
        return await func_async(x)
    ...
    @asyncToDeferred
    async def test_await_N_await_M_func_return(x):
        for i in range(n):
            x = await impl2(x)
        return x
    return test_await_N_await_M_func_return

def gen_test_bareawait_N_bareawait_M_func_return(n, m):
    ...
    @asyncToDeferred
    async def test_bareawait_N_bareawait_M_func_return(x):
        for i in range(n):
            x = await impl2(x)
        return x
    return test_bareawait_N_bareawait_M_func_return

def gen_test_yield_N_yield_M_deferred_return(n, m):
    ...
    @defer.inlineCallbacks
    def test_yield_N_yield_M_deferred_return(x):
        for i in range(n):
            x = yield impl2(x)
        return x
    return test_yield_N_yield_M_deferred_return

def gen_test_await_N_await_M_deferred_return(n, m):
    ...
    @asyncToDeferred
    async def test_await_N_await_M_deferred_return(x):
        for i in range(n):
            x = await impl2(x)
        return x
    return test_await_N_await_M_deferred_return

def gen_test_bareawait_N_bareawait_M_deferred_return(n, m):
    ...
    @asyncToDeferred
    async def test_bareawait_N_bareawait_M_deferred_return(x):
        for i in range(n):
            x = await impl2(x)
        return x
    return test_bareawait_N_bareawait_M_deferred_return
...
def do_tests(result_path):
    try:
        ...
        gen_test_functions = [
            ...
            (gen_test_yield_N_func_succeed_returnValue, 'test_N_func', 1, gen_args_n),
            (gen_test_yield_N_func_succeed_return, 'test_N_func', 1, gen_args_n),
            (gen_test_yield_N_func_returnValue, 'test_N_func', 1, gen_args_n),
            (gen_test_yield_N_func_return, 'test_N_func', 1, gen_args_n),
            (gen_test_yield_N_yield_M_func_return, 'test_N_M_func', 1, gen_args_n_m),
            (gen_test_await_N_await_M_func_return, 'test_N_M_func', 1, gen_args_n_m),
            (gen_test_bareawait_N_bareawait_M_func_return, 'test_N_M_func', 1, gen_args_n_m),
            (gen_test_addCallback_N_succeed_deferred, 'test_N_deferred', 1, gen_args_n),
            (gen_test_yield_N_succeed_deferred_return, 'test_N_deferred', 1, gen_args_n),
            (gen_test_yield_N_deferred_return, 'test_N_deferred', 1, gen_args_n),
            (gen_test_yield_N_yield_M_deferred_return, 'test_N_M_deferred', 1, gen_args_n_m),
            (gen_test_await_N_await_M_deferred_return, 'test_N_M_deferred', 1, gen_args_n_m),
            (gen_test_bareawait_N_bareawait_M_deferred_return, 'test_N_M_deferred', 1, gen_args_n_m),
        ]
...
def main():
    parser = argparse.ArgumentParser(prog='twisted_perf')
